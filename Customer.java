...
public class Customer extends Shop {
	...
	private int initializeProducts() throws IOException {
	    int x = 0;
	    try {
	        Class.forName("com.mysql.jdbc.Driver");
	        Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/onlineshop?autoReconnect=true&useSSL=false", "root", DatabaseConnection.root);
	        
	        // Change here to create a scrollable result set
	        PreparedStatement ps = con.prepareStatement("select * from products", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
	        ResultSet rs = ps.executeQuery();
	        ...
	        if (x == 0) {
	            ...
	            while (rs.next()) {
	                ...
	                qty.add(rs.getInt(4));
	                price.add(rs.getFloat(5));
	            }
	        }
	    } catch (Exception e) {
	        System.out.println("Error while initializing products: " + e.getMessage());
	        e.printStackTrace();
	    }
	    return x;
	}
